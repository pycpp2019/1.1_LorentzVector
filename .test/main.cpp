...
int main() {
    vector<pair<string, function<bool(Rng*)>>> tests = {
        ...
        make_pair(string("add"), [](Rng *rng) -> bool {
            for (int k = 0; k < N_TRIES; ++k) {
                LorentzVector lv0 = lv_random(rng), lv1 = lv_random(rng);
                LorentzVector lvs = lv0.add(lv1);
                if (!(
        ...
        make_pair(string("sub"), [](Rng *rng) -> bool {
            for (int k = 0; k < N_TRIES; ++k) {
                LorentzVector lv0 = lv_random(rng), lv1 = lv_random(rng);
                LorentzVector lvs = lv0.sub(lv1);
                if (!(
                    f_eq(lv0.t() - lv1.t(), lvs.t()) &&
                    f_eq(lv0.x() - lv1.x(), lvs.x()) &&
                    f_eq(lv0.y() - lv1.y(), lvs.y()) &&
        ...
        make_pair(string("mul"), [](Rng *rng) -> bool {
            for (int k = 0; k < N_TRIES; ++k) {
                LorentzVector lv = lv_random(rng);
                double f = rng->normal().first;
                LorentzVector lvs = lv.mul(f);
                if (!(
                    f_eq(lv.t()*f, lvs.t()) &&
                    f_eq(lv.x()*f, lvs.x()) &&
                    f_eq(lv.y()*f, lvs.y()) &&
        ...
        make_pair(string("zboost"), [](Rng *rng) -> bool {
            for (int k = 0; k < N_TRIES; ++k) {
                ...
                LorentzVector lv0 = lv_random(rng), lv1 = lv_random(rng);
                double norm = lv0.sub(lv1).norm();
                double beta = (1.0 - EPS)*2.0*(rng->uniform() - 0.5);
                lv0.zboost(beta);
                lv1.zboost(beta);
                if (!f_eq(lv0.sub(lv1).norm(), norm)) {
                    return false;
                }
            }
            return true;
        ...
        make_pair(string("print"), [](Rng *rng) -> bool {
            ...
            streambuf *stdoutbuf = cout.rdbuf(strout.rdbuf());

            bool res = true;
            for (int k = 0; k < N_TRIES; ++k) {
                LorentzVector lv = lv_random(rng).mul(100.0);
                lv.print();
                cout.flush();
        ...
        make_pair(string("read"), [](Rng *rng) -> bool {
            stringstream strio;
            streambuf *stdinbuf = cin.rdbuf(strio.rdbuf());
